...
const { filtersState, resetFilters, updateFilter, updateCustomPeriod, queryString } = useFilters();
...
const filters: Filter[] = [
  ...
  },
  {
    ...
    isActive: () => filtersState.value.selectedFilter === "5 минут",
  },
  {
    label: "10 минут",
    action: (event) => {
      const filter = "10 минут";
      if (isMobile.value) {
        selectedFilter.value = filter;
      } else {
        handleDesktopFilter(filter);
    ...
    isActive: () => filtersState.value.selectedFilter === "10 минут",
  },
  {
    label: "1 час",
    action: (event) => {
    ...
    isActive: () => filtersState.value.selectedFilter === "1 час",
  },
  {
    label: "За неделю",
    action: (event) => {
...
<template>
  ...
  >
    ...
    <img class="h-5 w-5 rotate-180 duration-300" :class="{ 'rotate-[360]': filterIsOpen === true }" src="~/assets/images/icons/chevron-down.svg" alt="" />
    <aside v-if="filterIsOpen && !isMobile" ref="menuRef" class="absolute z-20 -left-[1px] top-[100%] mt-[0.375rem] p-1 rounded-2xl bg-white menu-shadow">
      <div v-if="isPeriodSelected !== true" class="flex flex-col w-[11.875rem]">
        <div class="w-full" v-for="(filter, index) in filters" :key="index">
          <p
            @click.stop="filter.action"
  ...
  <div
    @click="closeMenu"
    v-if="filterIsOpen && isMobile"
    :class="{ 'bg-black/60': filterIsOpen }"
