...
const { fetchTrainingById, updateTrainingById } = useTrainings();
...
const { error } = await useAsyncData<any>(
  "contest",
  async () => {
    const response: any = await fetchTrainingById(Number(useRoute().params.id), { method: "GET" });

    trainingsTitle.value = response.title;
    trainingsSubtitle.value = response.subtitle;
...
const updateTrainings = async () => {
  ...
  try {
    await updateTrainingById(Number(useRoute().params.id), {
...
<template>
  <HeaderComponent :headerTitle="'Редактирование программы'" :cancel-link="'/education'" :button_status="disabledButton" @making-request="updateTrainings" />
  <MainComponent>
